...
<body>
    ...
    <li class="nav" style="float: center;"><u>Search Products</u></li>
  </ul>
    ...
    <table>
      ...
      <?php
        if (isset($_POST['products_sumbit'])) { // if user presses submit
          ...
          $sql = "Select * from products where product_id ='$search' or product_type LIKE '%$search%' or expiry_date LIKE '$search%'"; //query
            ...
            if (mysqli_num_rows($result) > 0) { //if there are more than zero rows with this id
              ...
              while ($row = mysqli_fetch_assoc($result)) { //display all the product details
                echo '
                    <tbody>
                    <tr>
                    <td><a href="..\productDetails.html">' . $row['product_id'] . '</a></td>
                    <td>' . $row['name'] . '</td>
